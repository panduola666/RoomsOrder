<template>
  <div class="d-md-none" v-if="roomInfo && roomInfo.imageUrlList && roomInfo.imageUrlList.length > 0">
    <swiper class="swiper" :modules="modules" :pagination="pagination" :navigation="navigation" :slidesPerView="1" :loop="isShowSlide">
...
const isShowSlide = ref<boolean>(false)

watchEffect(() => {
  const newWidth = window.innerWidth;
  if (newWidth > 767){
    return isShowSlide.value = false 
  }else{
    return isShowSlide.value = true
  }
});
